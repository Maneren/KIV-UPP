...
The simplest part of the program just reads the file line-by-line, splits each
line, and saves the data to a vector of Stations structs. Reading one file is
very hard to parallelize since it involves a lot of very small operations using
...
checked in parallel, but the (relatively fast) removal is better done later
...
for each month rather than each station. It is also very beneficial to
parallelize the file access since the compute part can run while another thread
is waiting for a file to finish writing to the drive.
...
varies wildly between the parts of the program. Also, surprisingly, the speedups
from parallelization are overall much more noticeable on the laptop, which has
fewer cores, but I double-checked the measurements and they seem accurate.

#### Metrics

##### Amdahl's law

According to Amdahl's law, the maximum speedup can be approximated as:

$$
\text{speedup} = \frac{1}{(1 - \text{parallelizable}) \cdot
    \frac{\text{parallelizable}}{\text{cores}}}
$$

Approximating from `perf` measurements:

- preprocessing is 99% parallelizable
- outlier detection is 98% parallelizable
- rendering is 60% parallelizable

That means the whole processing is (in theory) 82.8% parallelizable.
...
$$
\text{speedup} = \frac{1}{(1 - 0.828) + \frac{0.828}{16}} = 3.63
$$

which is way more than the measured speedup of 1.57.
